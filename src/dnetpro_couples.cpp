                       ...
                       )
{
  ...
  //std :: unique_ptr < int32_t[] > idx_sort_single  = nullptr; // sorted indices of single gene
  std :: unique_ptr < int32_t[] > idx_sort_couples = nullptr; // sorted indices of couples gene
  ...
  //const int32_t size_single       = diff_size_single  ? Nprobe - diff_size_single  : Nprobe;
  const int32_t diff_size_couples = Ncomb % nth;
  const int32_t size_couples      = diff_size_couples ? Ncomb  - diff_size_couples : Ncomb;
...
//  score single_gene(Nprobe, Nclass);
  ...
  score results(static_cast < int32_t >(Ncomb  * percentage), Nclass);
    ...
    for (int32_t gene_a = 0; gene_a < Nprobe; ++gene_a) // for each gene
      for (int32_t gene_b = 0; gene_b < Nprobe; ++gene_b) // for each gene
      {
        ...
        for (int32_t i = 0; i < Nsample; ++i) // looCV cycle
        {
          ...
          couples.class_score[predict_lbl][idx] += static_cast < int32_t >(labels[i] == predict_lbl);
        } // end sample loop
        ...
        couples.mcc[idx]  = score :: matthews_corrcoef(couples.class_score[0][idx],
                                                       static_cast < int32_t >(member_class[0].size()),
                                                       couples.class_score[1][idx],
                                                       static_cast < int32_t >(member_class[1].size()));
...
#pragma omp for private (idx, max_score, predict_lbl, tmp_a, tmp_b, count, mean_a, mean_b, var_a, var_b, discr)
    ...
    for (int32_t gene_a = 0; gene_a < Nprobe; ++gene_a) // for each gene
    {
      ...
      for (int32_t i = 0; i < Nsample; ++i) // looCV cycle
      {
        ...
        predict_lbl = predict_lbl < 0 ? 0 : predict_lbl;
        single_gene.class_score[predict_lbl][gene_a] += static_cast < int32_t >(labels[i] == predict_lbl);
      } // end sample loop
      // update total and gene number
      ...
      single_gene.mcc[gene_a]   = score :: matthews_corrcoef(single_gene.class_score[0][gene_a],
                                                             static_cast < int32_t >(member_class[0].size()),
                                                             single_gene.class_score[1][gene_a],
                                                             static_cast < int32_t >(member_class[1].size()));
...
#pragma omp single
    {
      mergeargsort_parallel_omp(idx_sort_single.get(), single_gene.tot.get(), 0, size_single, nth,
      ...
      if (diff_size_single)
      {
        std :: sort(idx_sort_single.get() + size_single, idx_sort_single.get() + Nprobe,
                    [&](const int32_t &a1, const int32_t &a2)
                    {
                      return single_gene.tot[a1] > single_gene.tot[a2];
                    });
        std :: inplace_merge(idx_sort_single.get(), idx_sort_single.get() + size_single, idx_sort_single.get() + Nprobe,
                             [&](const int32_t &a1, const int32_t &a2)
                             {
                               return single_gene.tot[a1] > single_gene.tot[a2];
                             });
    ...
    std :: sort(idx_sort_single.get(), idx_sort_single.get() + Nprobe,
                [&](const int32_t &a1, const int32_t &a2)
                {
                  return single_gene.tot[a1] > single_gene.tot[a2];
                });
...
#pragma omp single
    {
      mergeargsort_parallel_omp(idx_sort_couples.get(), couples.tot.get(), 0, size_couples, nth,
                                [&](const int32_t &a1, const int32_t &a2)
                                {
                                  return couples.tot[a1] > couples.tot[a2];
                                });
      if (diff_size_couples)
      {
        std :: sort(idx_sort_couples.get() + size_couples, idx_sort_couples.get() + Ncomb,
                    [&](const int32_t &a1, const int32_t &a2)
                    {
                      return couples.tot[a1] > couples.tot[a2];
                    });
        std :: inplace_merge(idx_sort_couples.get(), idx_sort_couples.get() + size_couples, idx_sort_couples.get() + Ncomb,
                             [&](const int32_t &a1, const int32_t &a2)
                             {
                               return couples.tot[a1] > couples.tot[a2];
                             });
    ...
    std :: sort(idx_sort_couples.get(), idx_sort_couples.get() + Ncomb,
                [&](const int32_t &a1, const int32_t &a2)
                {
                  return couples.tot[a1] > couples.tot[a2];
                });
...
#pragma omp for
#endif
    for (int32_t i = 0; i < static_cast < int32_t >(Ncomb * percentage); ++i)
    {
...
#pragma omp for
#endif
    for (int32_t i = 0; i < static_cast < int32_t >(Nprobe * percentage); ++i)
    {
      const int32_t idx = idx_sort_single[i];
