...
template < >
void ArgumentParser :: get < int32_t > (const std :: string & name, int32_t & values)
{
  for (std :: size_t i = 0; i < args.size(); ++i)
    if (args[i].name == name)
    {
      try
      {
        values = static_cast < int32_t >(std :: stod(args[i].values[0]));
        return;
      }
...
template < >
void ArgumentParser :: get < std :: vector < int32_t > > (const std :: string & name, std :: vector < int32_t > & values)
{
  bool found = true;
  ...
  for (std :: size_t i = 0; i < args.size(); ++i)
    if (args[i].name == name)
    {
      for (const auto & val : args[i].values)
      {
        try
        {
          values.push_back(static_cast < int32_t >(std :: stod(val)));
        }
        catch ( std :: invalid_argument & )
        {
