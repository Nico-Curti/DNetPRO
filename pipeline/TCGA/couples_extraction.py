...
def extract_couples (local, cancer, num_genes, percent=.1):
  '''
  ...
  for f in couple_files:
    ...
    couples = pd.DataFrame(np.fromfile(f, dtype=data_type).tolist(), columns=columns)[int(num_genes * percent) : ] # only mixed couples!

    #assert np.allclose(couples.genea, couples.geneb)
#    genes = couples.genea.values
#    genes = map(str, genes)
    best.append((f, couples.values))
...
if __name__ == '__main__':
    ...
    for cancer, num_genes in cancers.items():
      ...
      couples = extract_couples(local, cancer, num_genes)

      with open(cancer + '_kbest.pickle', 'wb') as fp:
        pickle.dump(couples, fp, pickle.HIGHEST_PROTOCOL)
